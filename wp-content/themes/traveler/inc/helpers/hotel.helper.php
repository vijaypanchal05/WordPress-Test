	...
	if(!class_exists('HotelHelper')){
		class HotelHelper{
			public function init(){
                ...
                add_action('wp_ajax_st_format_real_price', array(&$this, '_format_real_price'));
                add_action('wp_ajax_nopriv_st_format_real_price', array(&$this, '_format_real_price'));
			}
			...
			static function _check_room_available($room_id = '', $check_in = '', $check_out = '', $number_room = 0, $order_item_id = ''){
				...
				$sql = "
				SELECT _order.*,  _order.number_room - SUM(_order.room_num_search) AS free_room FROM (
						SELECT
				...
				HAVING _order.number_room - SUM(_order.room_num_search) < {$number_room}
					";
